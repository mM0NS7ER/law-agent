...
│   ├── test_small.json
│   ├── test_annotated.json
│   ├── test_small_annotated.json
│   ├── eval_retrieval.json
│   ├── eval_e2e_result.json
│   └── models/                     # 本地模型缓存（BAAI/bge-...）
│       └── ...
├── vector_store/              # 构建后的索引文件
├── build_index.py             # 索引构建脚本
├── build_annotation_file.py   # 标注构建脚本
...
| `qwen_model` | `qwen3.6-plus` | Qwen 模型名称 |
...
Qwen-3.6-plus 分类器判断用户问题是否属于刑法范畴。置信度低于阈值自动归为 `out_of_scope`。API 异常时降级为 `criminal_law`（可用回答优先）。
...
```bash
python evaluate.py --mode retrieval --annotated test_small_annotated.json --output data/eval_retrieval.json
```
...
当前状态：102 passed, ruff 零告警, mypy strict 模式零错误。
